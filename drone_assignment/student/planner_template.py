...
def choose_best_action(
    ...
    for action in env.available_actions(state):
        ...
        if action.value.startswith("MOVE") and next_pos == curr_pos:
            utility -= env.config.invalid_move_penalty
        ...
        if next_pos in env._map.hazards:
                utility -= C_HAZARD * 50 #adds penalty if in hazard
        ...
        if next_state.battery <= 0:
            if next_pos not in env._map.battery_stations:
                utility -= env.config.battery_depletion_penalty  # Avoid total depletion at all costs
            else:
        ...
        if action == Action.SCAN:
            if battery_urgent:
                ...
                if scan_count >= 2:
                    utility = -500.0
                if current_p > 0.95 or current_p < 0.05:
                    info_gain = -100.0
                else:
